...
const fmtDA = (n) => Number(n).toLocaleString('fr-FR');
const fmtDate = (s) => {
  if (!s) return '—';
  const [d] = s.split(' ');
  const parts = d.split('-');
...
const daysUntil = (s) => Math.ceil((new Date(s) - new Date('2026-06-21')) / 86400000);
...
/* ---- KPI card ---- */
function KPI({ label, value, unit, delta, icon, deltaTone = 'success' }) {
  return (
...
/* ---- Card / PageHeader ---- */
function Panel({ title, action, children, className = '', pad = true }) {
  return (
...
/* ---- Buttons ---- */
function AdminBtn({ children, variant = 'solid', size = 'md', icon, onClick, type = 'button', className = '', danger = false, disabled }) {
...
function LabeledInput({ label, value, onChange, type = 'text', placeholder, mono = false }) {
  return (
...
/* ---- Switch (focus ring offset so it never merges with the fill) ---- */
...
/* ---- DataTable (sortable) ---- */
function useTableSort(rows, initial) {
...
function DataTable({ columns, rows, sort, onSort, rowKey, empty = 'Aucun résultat.' }) {
  return (
...
function Drawer({ open, onClose, title, subtitle, children, footer, width = 440 }) {
  React.useEffect(() => {
...
/* ---- Modal (centered) ---- */
function Modal({ open, onClose, title, subtitle, icon, iconTone = 'info', children, footer, width = 420 }) {
  React.useEffect(() => {
    const onKey = (e) => e.key === 'Escape' && onClose();
    if (open) window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
...
/* ---- SVG charts ---- */
function AreaChart({ data, height = 150, color = '#1100FF', fmt = fmtDA }) {
  const w = 600, pad = 6;
...
function MiniSpark({ data, color = '#1100FF', height = 40 }) {
  const w = 120, pad = 2;
...
function BarRow({ label, value, max, suffix = '', barClass = 'bg-brand' }) {
  return (
    <div>
...
Object.assign(window, {
  fmtDA, fmtDate, daysUntil,
  Badge, KPI, Panel, PageHead, AdminBtn, SearchInput, Select, LabeledInput,
  Switch, Segmented, Tabs, useTableSort, DataTable, Drawer, Modal, useToast,
  AreaChart, MiniSpark, BarRow,
});
