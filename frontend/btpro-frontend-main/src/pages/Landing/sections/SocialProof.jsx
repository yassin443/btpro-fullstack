...
import { Icon, Container, Reveal } from '../_ui'
...
export default function SocialProof() {
  ...
  // Stats réelles (avec planchers marketing pour rester crédible au lancement)
  const { data } = useQuery({
    queryKey: ['stats-publiques'],
    queryFn: () => api.get('/cabinets/stats-publiques/').then(r => r.data),
    retry: false,
    staleTime: 10 * 60 * 1000,
  })
  const cabinets = Math.max(Number(data?.nb_cabinets) || 0, 120)
  const projets = Math.max(Number(data?.nb_projets) || 0, 1450)
  const architectes = Math.max(Number(data?.nb_architectes) || 0, 380)
      ...
      <Container>
        <Reveal delay={120}>
          <div className="mx-auto mt-12 grid max-w-3xl grid-cols-3 gap-4 rounded-2xl border border-hair bg-mist/50 px-4 py-8 sm:gap-8 sm:px-10">
            <Counter target={cabinets} suffix="+" label="Cabinets" />
            <Counter target={projets} suffix="+" label="Projets gérés" />
            <Counter target={architectes} suffix="+" label="Architectes" />
          </div>
        </Reveal>
      </Container>
    </section>
