...
import {
  cx, Icon, Reveal, AuthLayout, GoogleButton, Divider, ErrorBanner,
  Field, PasswordField, Spinner, strengthScore,
} from './_ui'
...
export default function Register() {
  ...
  const setFV = (k) => (e) => setF((v) => ({ ...v, [k]: e.target.value }))
  const setCV = (k) => (e) => setC((v) => ({ ...v, [k]: e.target.value }))
  const blur = (k) => () => setTouched((t) => ({ ...t, [k]: true }))

  const emailValid = EMAIL_REGEX.test(f.email)
  ...
  // Prefill from a Google sign-in that needs to complete registration (202)
  React.useEffect(() => {
    const st = location.state
    if (st?.google && st?.accessToken) {
      setGoogleToken(st.accessToken)
      setF((v) => ({ ...v, prenom: st.google.prenom || '', nom: st.google.nom || '', email: st.google.email || '' }))
      setStep(2)
    }
  }, [location.state])
      ...
      api.post('/users/google-auth/', { access_token: tokenResponse.access_token })
        .then((res) => {
          if (res.status === 202) {
            ...
            setF((v) => ({ ...v, prenom: res.data.prenom || '', nom: res.data.nom || '', email: res.data.email || '' }))
            setStep(2)
          } else {
  ...
  const pay = async () => {
    ...
    setError('')
    try {
      const fullPhone = c.tel ? `+213${c.tel.replace(/\D/g, '')}` : ''
      const PLAN = plan.toUpperCase()
      let access, user
      if (googleToken) {
        const res = await api.post('/users/google-auth/', { access_token: googleToken, plan: PLAN, nom_cabinet: c.cabinet })
        access = res.data.access; user = res.data.user
      } else {
        const res = await api.post('/users/register/', {
          prenom: f.prenom, nom: f.nom, email: f.email, password: f.pw,
          plan: PLAN, nom_cabinet: c.cabinet, telephone: fullPhone || undefined,
        })
        access = res.data.access; user = res.data.user
      }
      const checkout = await api.post('/finances/checkout/', { plan: PLAN }, { headers: { Authorization: `Bearer ${access}` } })
      ...
      if (d?.email) { setStep(1); setTouched((t) => ({ ...t, email: 1 })) }
      setPayLoading(false)
    }
  }
          ...
          <div>
            ...
            <div className="mt-6">
              ...
              <p className="mt-6 text-center text-[0.9rem] text-muted">
                Déjà un compte ? <button onClick={() => navigate('/login')} className="tap-ring rounded font-semibold text-brand hover:underline">Se connecter</button>
              </p>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            ...
            <div className="mt-4 flex gap-3">
              ...
              <PrimaryBtn onClick={pay} loading={payLoading} className="flex-1">{payLoading ? 'Redirection…' : <><Icon name="Lock" size={15} /> Payer {selectedPlan.price.toLocaleString('fr-FR')} DA</>}</PrimaryBtn>
            </div>
